...
if (process.argv[2] !== '--backup-f10') throw new Error('Explicit --backup-f10 required.');
verifyProject();
verifyLink();
const folder = resolve(process.env.LOCALAPPDATA, 'portfolio-alonso/backups/20260906-f10-analytics');
...
const evidence = {
  ...
  location: '%LOCALAPPDATA%/portfolio-alonso/backups/20260906-f10-analytics',
  files,
  scope:
...
writeFileSync('.tools/f10/backup-manifest.json', JSON.stringify(evidence, null, 2));
