...
writeFileSync('.tools/f11/local-schema.json', JSON.stringify(local, null, 2));
writeFileSync('.tools/f11/remote-schema.json', JSON.stringify(remote, null, 2));
const matches = {};
  ...
  try {
    if (key === 'functions') {
      ...
      assert.deepEqual(
        ...
        local[key].filter((f) => !isReservation(f)),
      );
      const before = remote[key].find(isReservation),
        after = local[key].find(isReservation);
      assert.deepEqual({ ...before, definition: null }, { ...after, definition: null });
      const source = readFileSync(
        'supabase/migrations/20260906002100_edge_build_contracts.sql',
        'utf8',
      );
      const expectedBody = source.split('as $$')[1].split('$$;')[0];
      const actualBody = before.definition.slice(
        before.definition.indexOf('declare'),
        before.definition.lastIndexOf('$function$'),
      );
      assert.equal(
        actualBody.trim().replace(/\s+/g, ' '),
        expectedBody.trim().replace(/\s+/g, ' '),
      );
    } else assert.deepEqual(remote[key], local[key]);
    matches[key] = true;
  } catch {
    matches[key] = false;
  }
...
writeFileSync('.tools/f11/remote-database.ts', generated);
assert.equal(
  generated,
  expected,
