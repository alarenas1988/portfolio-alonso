...
if (process.argv[2] !== '--verify-f7')
  throw new Error('Use --verify-f7 after local validation/deployment. Read-only.');
verifyProject();
verifyLink();
...
writeFileSync('.tools/f7/local-schema.json', JSON.stringify(local, null, 2));
writeFileSync('.tools/f7/remote-schema.json', JSON.stringify(remote, null, 2));
const matches = {};
...
writeFileSync('.tools/f7/remote-database.ts', generated);
assert.equal(
  generated,
  expected,
...
writeFileSync('.tools/f7/post-deploy.json', JSON.stringify(evidence, null, 2));
