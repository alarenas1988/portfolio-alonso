...
Las suites generales continúan usando fixtures públicos aislados y Analytics deshabilitado. El [checkpoint de cierre F7](checkpoints/F7_ADMIN_CMS.md) registra pruebas locales, revisión de capturas, validación remota y límites. No considerar un shell visible como evidencia de autorización.

El flujo email/password y recovery PKCE se probaron con Auth local real. La comprobación remota reutilizó el canal administrativo de enlace de un solo uso de los checkpoints aprobados: intercambio en memoria por una sesión real del owner, sin enviar correo, obtener su contraseña ni cambiar email/UUID/role/active. El navegador recibió solamente esa sesión a través del SDK oficial, nunca una clave privilegiada. Guardó un proyecto temporal y consultó el reporte privado real. No se afirma haber automatizado el formulario de contraseña del owner remoto.

023 quedó aplicada remotamente después de la puerta local y el dry-run. Se comprobaron 34 condiciones remotas y la limpieza posterior: cero proyectos/posts/experiencias/tecnologías/media/objetos/mensajes/eventos/builds de prueba y un único owner original. La comparación de catálogo, tipos y permisos no detectó drift de aplicación. El CMS permanece en esta rama para revisión, sin publicación de GitHub Pages.
